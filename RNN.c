...
void train(double input[], double target[], double prev_hidden_output[], double hidden_output[], double *total_error) {
    ...
    for (int i = 0; i < HIDDEN_NODES; i++) {
        for (int j = 0; j < HIDDEN_NODES; j++) {
            hidden_layer.recurrent_weights[i][j] += LEARNING_RATE * hidden_error[j] * prev_hidden_output[i];
        }
    }
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        for (int clue = 0; clue < num_clues; clue++) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                train(input, target, prev_hidden_output, hidden_output, &total_error);
